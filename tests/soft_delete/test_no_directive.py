...
def test_delete_hard_deletes_without_directive(no_sd_dbsession):
    ...
    no_sd_dbsession.add(task)
    no_sd_dbsession.flush()
    task_id = task.id

    no_sd_dbsession.delete(task)
    no_sd_dbsession.flush()

    row = no_sd_dbsession.execute(
...
def test_query_returns_all_without_directive(no_sd_dbsession):
    ...
    results = no_sd_dbsession.execute(select(Task)).scalars().all()
